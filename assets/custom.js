...
// ===========================================
// 10. FIX HERO NAVIGATION CONFLICT
// ===========================================

// Store hero ScrollTrigger reference when created
document.addEventListener("DOMContentLoaded", function() {
  // Find and store hero ScrollTrigger after it's created
  setTimeout(() => {
    const allTriggers = ScrollTrigger.getAll();
    heroScrollTrigger = allTriggers.find(trigger => 
      trigger.trigger && trigger.trigger.classList.contains('hero-section')
    );
  }, 1000);
});

// Override scrollToSection with hero conflict fix
window.scrollToSection = function(targetId) {
  ...
  // Temporarily disable hero ScrollTrigger during navigation
  ...
  const scrollTl = gsap.timeline({
    onComplete: () => {
      // Re-enable hero ScrollTrigger after navigation complete
      if (heroScrollTrigger) {
        setTimeout(() => {
